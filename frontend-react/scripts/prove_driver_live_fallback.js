...
(async () => {
  ...
  await page.screenshot({ path: '/tmp/driver-live-fallback.png', fullPage: true });
  ...
  const output = {
    ...
    tableCount,
    menuButtons,
    screenshot: '/tmp/driver-live-fallback.png',
  };

  fs.writeFileSync('/tmp/driver-live-fallback.json', JSON.stringify(output, null, 2));
  fs.mkdirSync(proofsDir, { recursive: true });
  fs.copyFileSync('/tmp/driver-live-fallback.json', path.join(proofsDir, 'prove_driver_live_fallback.json'));
  fs.copyFileSync('/tmp/driver-live-fallback.png', path.join(proofsDir, 'prove_driver_live_fallback.png'));
