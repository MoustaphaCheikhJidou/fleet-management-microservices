...
async function run() {
  ...
  await navigateDashboard(page, 'Espace Exploitant');
  await page.waitForSelector('#section-invoices table', { timeout: 12000 });

  const bodyText = await page.textContent('body');
  const currencyMatch = bodyText.match(/MAD|DH/);
  const billingRows = await page.$$eval('#section-invoices tbody tr', (els) => els.length);
  const output = {
    url: page.url(),
    titlePresent: await page.locator('text=Espace Exploitant').first().isVisible().catch(() => false),
    roleHeaderOk: await page.locator('text=Espace Exploitant').first().isVisible().catch(() => false),
  ...
  const screenshotTmp = '/tmp/operator-account-proof.png';
  const jsonTmp = '/tmp/operator-account-proof.json';
  await page.screenshot({ path: screenshotTmp, fullPage: true });
  fs.writeFileSync(jsonTmp, JSON.stringify(output, null, 2));
  fs.mkdirSync(proofsDir, { recursive: true });
  fs.copyFileSync(jsonTmp, path.join(proofsDir, 'operator-account-proof.json'));
  fs.copyFileSync(screenshotTmp, path.join(proofsDir, 'operator-account-proof.png'));
  ...
  bail('Preuve exploitant invalide', output);
}
