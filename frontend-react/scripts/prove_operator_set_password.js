...
const PASSWORD = process.env.OP_PASSWORD || 'Operator#123';
...
async function main() {
  const inviteEmail = process.env.OP_EMAIL || readFromTmp('/tmp/operator_invite_email.txt');
  if (!inviteEmail) throw new Error('Missing operator invite email. Run prove_admin_invite_operator first.');

  const mail = await waitForMail({ to: inviteEmail, subjectSubstring: 'Invitation' });
  const resetUrl = mail.resetUrl || readFromTmp('/tmp/operator_reset_url.txt');
  ...
  const screenshotPath = '/tmp/prove_operator_set_password.png';
  ...
  const jsonPath = '/tmp/prove_operator_set_password.json';
